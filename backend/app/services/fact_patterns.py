...
# 中文财报叙述中的连接词：指标名和数值之间可出现"为/约/约人民币/人民币"等。
# 「约人民币」是年报管理层讨论中极常见的表述，之前遗漏导致 634 亿未被抽取。
_C = r"(?:约人民币|约为|约|人民币|为|达到|实现|超|超过|高于|达|：|:)?"

FACT_RULES: tuple[FactRule, ...] = (
    FactRule(
        pattern=r"(?:研发费用)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="R&D_expenditure",
    ),
    FactRule(
        pattern=r"(?:研发投入(?!占))" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="R&D_total_spending",
    ),
    FactRule(
        pattern=(
            r"(?:"
            + VALUE_UNIT_PATTERN
            + r"\s*(?:用于|投向)?(?:研发费用|研发)|研发(?:费用)?(?:超|超过|高于|达)\s*"
    ...
    FactRule(
        pattern=(
            r"(?:"
            + VALUE_UNIT_PATTERN
            + r"\s*(?:用于|投向)?研发投入(?!占)|研发投入(?:超|超过|高于|达)\s*"
            + ALT_VALUE_UNIT_PATTERN
            + r")"
        ),
        metric_name="R&D_total_spending",
    ...
    FactRule(
        pattern=r"(?:营收|营业收入|营业总收入)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="revenue",
    ),
    FactRule(
        pattern=r"(?:归母净利润|归属于上市公司股东的净利润)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="net_profit_parent",
    ),
    FactRule(
        pattern=r"(?:扣非净利润|扣非归母净利润|扣除非经常性损益后的净利润)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="net_profit_deducted",
    ),
    FactRule(
        pattern=r"(?:净利润)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="net_profit",
    ),
    FactRule(
        pattern=r"(?P<segment>[一-龥A-Za-z0-9、及与/（）() -]{2,40})(?:收入|销售收入)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="revenue_segment",
    ),
    FactRule(
        pattern=r"(?P<product>[一-龥A-Za-z0-9、及与/（）() -]{2,30})?(?:产能状况|产能)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="production_capacity",
    ),
    FactRule(
        pattern=r"(?P<product>[一-龥A-Za-z0-9、及与/（）() -]{2,30})?(?:产量|快报产量)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
        metric_name="production_volume",
    ),
    FactRule(
        pattern=r"(?P<product>[一-龥A-Za-z0-9、及与/（）() -]{2,30})?(?:销量|快报销量)" + _C + r"\s*" + VALUE_UNIT_PATTERN,
